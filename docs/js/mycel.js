...
function switchOsTab(btn, os) {
  ...
  btn.classList.add('active');
  document.querySelectorAll('#os-win, #os-unix, #run-win, #run-unix, #update-win, #update-unix').forEach(el => el.hidden = true);
  document.getElementById('os-' + os).hidden = false;
  document.getElementById('run-' + os).hidden = false;
  document.getElementById('update-' + os).hidden = false;
  updateURL();
}
